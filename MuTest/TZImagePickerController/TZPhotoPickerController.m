...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = MUColor(238, 238, 238);
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didSelectPhoto:) name:@"PhotoPickPreviewSelectPhoto" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didDeselectPhoto:) name:@"PhotoPickPreviewDeselectPhoto" object:nil];
}

- (void)createTitleBtn
{
    self.titleBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 150, 44)];
    _titleBtn.titleLabel.font = MUFont(17);
    [_titleBtn setTitleColor:MUColor(34, 36, 38) forState:UIControlStateNormal];
    [_titleBtn setTitle:[NSString stringWithFormat:@"%@", _model.name] forState:UIControlStateNormal];
    [_titleBtn addTarget:self action:@selector(callAlbumView:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)configCollectionView {
    ...
    _collectionView.backgroundColor = MUColor(238, 238, 238);
...
- (void)configBottomToolBar {
    ...
    _bottomToolBar.tag = 99;
    CGFloat rgb = 253 / 255.0;
    _bottomToolBar.backgroundColor = MUColor(249, 249, 249);
    
    
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _doneButton.titleLabel.font = [UIFont systemFontOfSize:14];
    _doneButton.backgroundColor = MUColor(34, 36, 38);
...
//    [_bottomToolBar addSubview:_previewButton];
    ...
    [_bottomToolBar addSubview:_imageScroll];
//    [_bottomToolBar addSubview:_numberImageView];
//    [_bottomToolBar addSubview:_numberLabel];
    
    [myDelegate.window addSubview:_bottomToolBar];
...
- (void)viewDidLayoutSubviews {
    ...
    _doneButton.frame = CGRectMake(self.view.tz_width - 80 - 15, 10, 80, 30);
    _imageScroll.frame = CGRectMake(15, 0, ScreenWidth - 120, 50);
...
- (void)refreshBottomToolBarStatus {
    ...
    [_doneButton setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)", tzImagePickerVc.doneBtnTitleStr, tzImagePickerVc.selectedModels.count, tzImagePickerVc.maxImagesCount] forState:UIControlStateNormal];
    
    CGFloat right = 0;
    for (int i = 0; i < _imageScroll.subviews.count; i++) {
        UIImageView *iv = _imageScroll.subviews[i];
        iv.y = 8;
        iv.x = 5 + i * 45;
        right = iv.right + 5;
    }
    if (right > _imageScroll.width) {
        _imageScroll.contentSize = CGSizeMake(right, 0);
    }else
    {
        _imageScroll.contentSize = CGSizeMake(_imageScroll.width, 0);
    }
    
}

- (void)removePhoto:(TZAssetModel *)model
{
    NSInteger index = [_selectedPhotos indexOfObject:model];
    [[_imageScroll.subviews objectAtIndex:index] removeFromSuperview];
    [_selectedPhotos removeObject:model];
}
- (void)addPhoto:(TZAssetModel *)model
{
    __block UIImageView *iv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 35, 35)];
...
{
    [self addPhoto:notification.object];
    [self refreshBottomToolBarStatus];
}

- (void)didDeselectPhoto:(NSNotification *)notification
{
    [self removePhoto:notification.object];
    [self refreshBottomToolBarStatus];
}
